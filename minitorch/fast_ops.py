...
def njit(fn: Fn, **kwargs: Any) -> Fn:
    """Just-in-time compile a function with 'always' inline optimization."""
...
def tensor_map(
    ...
    * When `out` and `in` are stride-aligned, avoid indexing
    ...
    def _map(
        ...
        # If strides aligned, map directly
...
def tensor_zip(
    ...
    * When `out`, `a`, `b` are stride-aligned, avoid indexing
    ...
    def _zip(
        ...
        # If tensors are aligned, zip directly
        if (
    ...
    def _reduce(
        ...
        for i in prange(out.size):
            ...
            for j in range(a_shape[reduce_dim]):
                current_pos = a_pos + j * a_strides[reduce_dim]
                reduce_val = fn(float(a_storage[current_pos]), float(reduce_val))
