...
class CNNSentimentKim(minitorch.Module):
    ...
    def __init__(
        ...
        self.classes = 1
        self.conv1dsize3 = Conv1d(embedding_size, feature_map_size, filter_sizes[0])
        self.conv1dsize4 = Conv1d(embedding_size, feature_map_size, filter_sizes[1])
        self.conv1dsize5 = Conv1d(embedding_size, feature_map_size, filter_sizes[2])
    ...
    def forward(self, embeddings):
        ...
        c1 = self.conv1dsize3.forward(embeddings).relu()
        c2 = self.conv1dsize4.forward(embeddings).relu()
        c3 = self.conv1dsize5.forward(embeddings).relu()
        out = (minitorch.max(c1, 2) + minitorch.max(c2, 2) + minitorch.max(c3, 2)).view(embeddings.shape[0], self.feature_map_size)
